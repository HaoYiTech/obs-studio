...
export default class FileManager {
  ...
  /**
   * 接收到消息时，通过UI类的管理进行渲染
   * @param msg 接收到的消息，这个对象应是由createNormalChatItem()方法生成的。
   */
  showMsg({msg}) {
  ...
  _sendFileMsg({content, duration, type}) {
    const temp = this._page.createNormalChatItem({type, content, duration});
  ...
  uploadFileAndSend({content, duration, type, itemIndex}) {
    this._page.simulateUploadFile({
      ...
      success: (content) => {
        this._page.sendMsg({
          content: this._page.createChatItemContent({
            type,
            content,
            duration
          }),
