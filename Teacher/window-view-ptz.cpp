...
void CPTZWindow::onTriggerCameraPTZ(int nDBCameraID)
{
...
void CPTZWindow::initWindow()
{
	...
	// 设置窗口背景透明 => 设置之后会造成全黑问题 => 顶层窗口需要用背景蒙板...
	...
	// 必须使用背景蒙板才能解决顶层窗口的圆角问题...
	QPalette myPalette;
	QPixmap  myPixmap = QPixmap(":/ptz/images/ptz/back.png").scaled(this->size());
	// 让背景图片适应窗口的大小，在背景图片上做 scaled 操作...
	myPalette.setBrush(QPalette::Background, QBrush(myPixmap));
	this->setPalette(myPalette);
	this->setMask(myPixmap.mask());
...
{
	return App()->doSendCameraPTZCmd(m_nDBCameraID, inCMD, inSpeedVal);
}
