...
void EnumProfiles(std::function<bool (const char *, const char *)> &&cb)
{
	...
	int ret = GetConfigPath(path, sizeof(path), "obs-teacher/basic/profiles/*");
	if (ret <= 0) {
		...
		const char *oldName = nullptr)
{
	...
	ret = GetConfigPath(path, sizeof(path), "obs-teacher/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
static bool CopyProfile(const char *fromPartial, const char *to)
{
	...
	ret = GetConfigPath(dir, sizeof(dir), "obs-teacher/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
		...
		const char *init_text)
{
	...
	char baseDir[512];
	int ret = GetConfigPath(baseDir, sizeof(baseDir), "obs-teacher/basic/profiles/");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return false;
	}
...
void OBSBasic::DeleteProfile(const char *profileName, const char *profileDir)
{
	...
	int ret = GetConfigPath(basePath, 512, "obs-teacher/basic/profiles");
	if (ret <= 0) {
		blog(LOG_WARNING, "Failed to get profiles config path");
		return;
	}
...
void OBSBasic::on_actionImportProfile_triggered()
{
	...
	int ret = GetConfigPath(path, 512, "obs-teacher/basic/profiles/");
	if (ret <= 0) {
