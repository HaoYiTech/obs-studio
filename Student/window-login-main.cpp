...
void CRoomItem::paintEvent(QPaintEvent *event)
{
	...
	painter.drawText(nXPos, nYPos, QString("%1%2").arg(QStringLiteral("教室：")).arg(nLiveRoomID));
...
void LoginWindow::onTriggerRoomList(int nRoomPage)
{
	...
	if (m_strUTF8Data.size() <= 0) {
		ui->errLabel->setText(QTStr("Student.Room.List"));
		return;
	}
	// 开始解析传递过来的 json 数据包...
...
void LoginWindow::doLoginAction(int nLiveRoomID)
{
	...
	json_decref(theRoot);
	// 通知主进程主窗口可以启动了，传递房间号...
	emit this->doTriggerLoginSuccess(strLiveRoomID);
}
//
// 响应房间对象被点击事件...
